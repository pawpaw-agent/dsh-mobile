...
> 记录时间：1.11.0（versionCode 28，commit `45925ee`）；第四节补记于日志版（commit `17e3e03`）
> 背景：1.10.0 把 SSH 隧道所有权上移到 `DshApp`，1.11.0 删除了后台通知与其前台服务。
> 这两个版本的**真机验证尚未完成**（等待手机恢复无线调试），下面两条是按代码推导出的
> 待验证/待修项，**暂不改动**，避免在 1.11.0 上机前叠加未验证的改动。
...
（`DshMobile` / `DshApp` / `SshTunnel` / `TuiActivity`）。按文件看问题很集中：
